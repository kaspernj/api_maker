...
bootstrap_package["dependencies"]["@kaspernj/api-maker-inputs"] = api_maker_new_version
...
table_package["dependencies"]["@kaspernj/api-maker-bootstrap"] = api_maker_new_version
table_package["dependencies"]["@kaspernj/api-maker-inputs"] = api_maker_new_version
...
dummy_package["dependencies"]["@kaspernj/api-maker-bootstrap"] = api_maker_new_version
dummy_package["dependencies"]["@kaspernj/api-maker-inputs"] = api_maker_new_version
dummy_package["dependencies"]["@kaspernj/api-maker-table"] = api_maker_new_version

File.write("ruby-gem/spec/dummy/package.json", JSON.pretty_generate(dummy_package))


# RELEASE NEW VERSIONS
#run_command_with_retry("cd api_maker_table_npm && yarn && yarn publish --new-version #{api_maker_new_version} --otp #{otp}")
#wait_for_publish

#run_command_with_retry("cd npm-api-maker && yarn publish --new-version #{api_maker_new_version} --otp #{otp}")
#wait_for_publish

#run_command_with_retry("cd npm-api-maker-inputs && yarn && yarn publish --new-version #{api_maker_new_version} --otp #{otp}")
#wait_for_publish

#run_command_with_retry("cd npm-api-maker-bootstrap && yarn && yarn publish --new-version #{api_maker_new_version} --otp #{otp}")
#wait_for_publish
