...
const validateRspackDependencies = () => {
    ...
    const missingDependencies = requiredDependencies.filter((dep) => !moduleExists(dep));
    if (missingDependencies.length > 0) {
        error(`Missing required dependencies for RSpack:\n${missingDependencies
            .map((dep) => `  - ${dep}`)
            .join("\n")}\n\nPlease install them with:\n  npm install ${missingDependencies.join(" ")}`);
        throw new Error(`Missing RSpack dependencies: ${missingDependencies.join(", ")}`);
    }
};
const validateWebpackDependencies = () => {
    ...
    const missingDependencies = requiredDependencies.filter((dep) => !moduleExists(dep));
    if (missingDependencies.length > 0) {
        error(`Missing required dependencies for Webpack:\n${missingDependencies
            .map((dep) => `  - ${dep}`)
            .join("\n")}\n\nPlease install them with:\n  npm install ${missingDependencies.join(" ")}`);
        throw new Error(`Missing Webpack dependencies: ${missingDependencies.join(", ")}`);
    }
};
module.exports = {
