...
class BuildValidator {
    constructor(options) {
        this.options = {
            ...
            strictBinaryResolution: options.strictBinaryResolution ||
                process.env.CI === "true" ||
            ...
            if (SAFE_ENV_VARS.includes(key)) {
                ...
                if (SUSPICIOUS_ENV_PATTERN.test(value)) {
                    if (this.options.verbose) {
                        console.warn(`   [Security Warning] Suspicious pattern detected in environment variable ${key}: ${value}`);
                    }
                }
                filtered[key] = value;
            }
     ...
     */
    static validateConfigPath(configFile, appRoot, buildName) {
        const configPath = (0, path_1.resolve)(appRoot, configFile);
        ...
        const rel = (0, path_1.relative)(appRoot, configPath);
        // Path is valid if:
        ...
        if (rel !== "" && rel.startsWith("..")) {
            throw new Error(`Invalid config file path for build '${buildName}': Path must be within project directory. ` +
                `Config file: ${configFile}, Resolved path: ${configPath}, Project root: ${appRoot}`);
        }
        if (!(0, fs_1.existsSync)(configPath)) {
            throw new Error(`Config file not found for build '${buildName}': ${configPath}. ` +
     ...
     */
    async validateBuild(build, appRoot) {
     ...
     */
    async validateHMRBuild(build, appRoot, bundler) {
        ...
        const devServerCmd = bundler === "rspack" ? "rspack-dev-server" : "webpack-dev-server";
        const devServerBin = this.findBinary(devServerCmd, appRoot);
        if (!devServerBin) {
            const packageManager = (0, fs_1.existsSync)((0, path_1.resolve)(appRoot, "yarn.lock"))
                ? "yarn add"
                : "npm install";
            result.errors.push(`Could not find ${devServerCmd} binary. Please install it:\n` +
        ...
        if (build.configFile) {
            try {
                const configPath = BuildValidator.validateConfigPath(build.configFile, appRoot, build.name);
            ...
            const defaultConfig = (0, path_1.resolve)(appRoot, `config/${bundler}/${bundler}.config.js`);
        ...
        return new Promise((resolvePromise) => {
            const child = (0, child_process_1.spawn)(devServerBin, args, {
                cwd: appRoot,
            ...
            const timeoutId = setTimeout(() => {
                if (!hasCompiled && !resolved && !processKilled) {
                    result.errors.push(`Timeout: webpack-dev-server did not compile within ${this.options.timeout}ms.`);
                    processKilled = true;
                    child.kill("SIGTERM");
            ...
            const processOutput = (data) => {
                ...
                lines.forEach((line) => {
                    ...
                    const isErrorOrWarning = line.trim().startsWith("ERROR") || line.trim().startsWith("WARNING");
                    if (!processKilled &&
                        !isErrorOrWarning &&
                        ...
                        setTimeout(() => {
                            if (!resolved) {
                                if (this.options.verbose) {
                                    console.warn(`   [Warning] Process did not exit after SIGTERM, forcing resolution.`);
                                }
                                child.stdout?.removeAllListeners();
                                child.stderr?.removeAllListeners();
                                child.removeAllListeners();
            ...
            child.on("exit", (code) => {
                ...
                if (!hasCompiled && !hasError && !resolved) {
                    if (code !== 0 && code !== null && code !== SIGTERM_EXIT_CODE) {
                        result.errors.push(`${devServerCmd} exited with code ${code} before compilation completed.`);
                    }
                }
                resolveOnce(result);
            });
     ...
     */
    async validateStaticBuild(build, appRoot, bundler) {
        const startTime = Date.now();
        const result = {
            buildName: build.name,
            success: false,
        ...
        if (!bundlerBin) {
            const packageManager = (0, fs_1.existsSync)((0, path_1.resolve)(appRoot, "yarn.lock"))
                ? "yarn add"
                : "npm install";
            result.errors.push(`Could not find ${bundler} binary. Please install it:\n` +
        ...
        args.push("--json");
        return new Promise((resolvePromise) => {
            const child = (0, child_process_1.spawn)(bundlerBin, args, {
                cwd: appRoot,
                env: this.filterEnvironment(build.environment),
                stdio: ["ignore", "pipe", "pipe"]
            });
            ...
            const timeoutId = setTimeout(() => {
                result.errors.push(`Timeout: ${bundler} did not complete within ${this.options.timeout}ms.`);
                child.kill("SIGTERM");
            ...
            child.on("exit", (code) => {
                ...
                try {
                    ...
                    if (jsonOutput.warnings && jsonOutput.warnings.length > 0) {
                        jsonOutput.warnings.forEach((warning) => {
                            ...
                            if (typeof warning === "string") {
                                ...
                                try {
                                    warningMsg = JSON.stringify(warning, Object.getOwnPropertyNames(warning));
                                }
                                catch {
                    ...
                    if (code !== 0 && result.errors.length === 0) {
                        result.errors.push(`${bundler} exited with code ${code} but no errors were captured. ` +
     ...
     */
    findBinary(name, appRoot) {
        // Try node_modules/.bin (preferred for security)
        const nodeModulesBin = (0, path_1.resolve)(appRoot, "node_modules", ".bin", name);
        if ((0, fs_1.existsSync)(nodeModulesBin)) {
            return nodeModulesBin;
        }
        // Try global installation
        const globalBin = (0, path_1.resolve)("/usr/local/bin", name);
        if ((0, fs_1.existsSync)(globalBin)) {
            if (this.options.verbose) {
                console.log(`   [Security Warning] Using global ${name} from /usr/local/bin. ` +
        ...
        if (this.options.verbose) {
            console.log(`   [Security Warning] Binary '${name}' not found locally. ` +
    ...
    formatResults(results) {
        ...
        lines.push(`Summary: ${successCount}/${totalBuilds} builds passed, ${failureCount} failed (Total: ${totalSeconds}s)`);
        lines.push("=".repeat(80));
        // Add debugging guidance if there are failures
        if (failureCount > 0) {
            lines.push("\n💡 Debugging Tips:");
            lines.push("   To get more details, run individual builds with --verbose:");
            lines.push("");
            const failedBuilds = results.filter((r) => !r.success);
            failedBuilds.forEach((result) => {
                lines.push(`   bin/shakapacker-config --validate-build ${result.buildName} --verbose`);
            });
            lines.push("");
            lines.push("   Or validate all builds with full output: bin/shakapacker-config --validate --verbose");
            lines.push("=".repeat(80));
        }
        lines.push("");
        return lines.join("\n");
