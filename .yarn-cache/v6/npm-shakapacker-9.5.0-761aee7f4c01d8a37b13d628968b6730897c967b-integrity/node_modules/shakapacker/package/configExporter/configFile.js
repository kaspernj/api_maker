...
exports.generateSampleConfigFile = generateSampleConfigFile;
...
class ConfigFileLoader {
    ...
    constructor(configFilePath) {
        this.configFilePath =
            configFilePath || (0, path_1.resolve)(process.cwd(), types_1.DEFAULT_CONFIG_FILE);
     ...
     */
    validateConfigPath() {
        const absPath = (0, path_1.resolve)(this.configFilePath);
        ...
        try {
            ...
            if ((0, fs_1.existsSync)(absPath)) {
                realPath = (0, fs_1.realpathSync)(absPath);
            }
            else {
        ...
        const rel = (0, path_1.relative)(cwd, realPath);
        if (rel.startsWith("..") ||
            ((0, path_1.isAbsolute)(rel) && !realPath.startsWith(cwd))) {
            throw new Error(`Config file must be within project directory. Attempted path: ${this.configFilePath}`);
        }
    }
    /**
     ...
     */
    exists() {
        return (0, fs_1.existsSync)(this.configFilePath);
    }
    /**
    ...
    load() {
        ...
        try {
            const content = (0, fs_1.readFileSync)(this.configFilePath, "utf8");
            ...
            throw new Error(`Failed to load config file ${this.configFilePath}: ${errorMessage}`);
        }
    }
    static validate(config) {
        ...
        if (config.default_bundler &&
            config.default_bundler !== "webpack" &&
            config.default_bundler !== "rspack") {
            throw new Error(`Invalid default_bundler '${config.default_bundler}'. Must be 'webpack' or 'rspack'.`);
        }
        // Validate each build
        for (const [name, build] of Object.entries(config.builds)) {
            ...
            if (build == null || typeof build !== "object" || Array.isArray(build)) {
                ...
                throw new Error(`Invalid build '${name}': must be an object, got ${buildType}`);
            }
            if (build.bundler &&
                build.bundler !== "webpack" &&
                build.bundler !== "rspack") {
                throw new Error(`Invalid bundler '${build.bundler}' in build '${name}'. Must be 'webpack' or 'rspack'.`);
            }
     ...
     */
    resolveBuild(buildName, options, defaultBundler) {
        const config = this.load();
        const build = config.builds[buildName];
        if (!build) {
            const available = Object.keys(config.builds).join(", ");
            throw new Error(`Build '${buildName}' not found in config file.\n` +
                `Available builds: ${available}\n` +
                `Use --list-builds to see all available builds.`);
        }
        // Resolve bundler with precedence
        const bundler = ConfigFileLoader.resolveBundler(options.bundler, build.bundler, config.default_bundler, defaultBundler);
        // Expand environment variables
        const environment = this.expandEnvironmentVariables(build.environment || {}, bundler);
        // Convert bundler_env to CLI args
        const bundlerEnvArgs = ConfigFileLoader.convertBundlerEnvToArgs(build.bundler_env || {});
        // Resolve and validate outputs
        const outputs = build.outputs || [];
        // Validate edge cases
        if (outputs.length === 0) {
            throw new Error(`Build '${buildName}' has empty outputs array. ` +
                `Please specify at least one output type (client, server, or all).`);
        }
        // Check for duplicates
        const uniqueOutputs = new Set(outputs);
        if (uniqueOutputs.size !== outputs.length) {
            throw new Error(`Build '${buildName}' has duplicate output types. ` +
                `Each output type should appear only once.`);
        }
        // Resolve config file
        let configFile;
        if (build.config) {
            configFile = this.expandEnvironmentVariables({ config: build.config }, bundler).config;
            ...
            if (configFile) {
                ...
                if (configFileNormalized.includes("..") ||
                    !configFileNormalized.startsWith("config/")) {
                    throw new Error(`Invalid config file path in build '${buildName}': "${configFile}". ` +
    ...
    static resolveBundler(cliFlag, buildBundler, defaultBundler, fallback = "webpack") {
        return cliFlag || buildBundler || defaultBundler || fallback;
    }
    expandEnvironmentVariables(vars, bundler) {
        const expanded = {};
    ...
    expandString(str, bundler) {
        ...
        expanded = expanded.replace(/\$\{([^}:]+):-([^}]*)\}/g, (_, varName, defaultValue) => {
            // Validate env var name to prevent regex injection
            if (!ConfigFileLoader.isValidEnvVarName(varName)) {
                console.warn(`[Config Exporter] Warning: Invalid environment variable name: ${varName}`);
    ...
    static convertBundlerEnvToArgs(bundlerEnv) {
        const args = [];
    ...
    listBuilds() {
        ...
        for (const [name, build] of Object.entries(builds)) {
            const bundler = build.bundler || config.default_bundler || "webpack (default)";
 ...
 */
function generateSampleConfigFile(ssr = false) {
 ...
 * @returns YAML content as string ready to be written to file
 */
function generateSSRConfigFile() {
