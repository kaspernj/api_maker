class ApiMaker::TranslatedCollections
  ...
  def self.add(allow_blank:, blk:, collection_name:, helper_methods:, helper_methods_prepend: false, model_class:, validate_inclusion:) # rubocop:disable Metrics/AbcSize
    ...
    I18n.available_locales.each do |locale|
      I18n.with_locale(locale) do
        ...
        collections[locale.to_s] = {
          collection:,
          collection_array: collection.map { |translation, value| [translation:, value:] }.flatten,
    ...
    model_class.validates collection_name, allow_blank: allow_blank, inclusion: {in: collection_values} if validate_inclusion
  end
  ...
  def self.add_with_scope(model_class, collection_name, plural_name, collection_values_as_strings)
    model_class.scope :"with_#{plural_name}", lambda { |*options|
  ...
  def self.add_without_scope(model_class, collection_name, plural_name, collection_values_as_strings)
    model_class.scope :"without_#{plural_name}", lambda { |*options|
      # Check that options are valid
      options.each do |option|
        raise InvalidCollectionValueError, "Invalid option for #{collection_name}: #{option}" if collection_values_as_strings.exclude?(option.to_s)
      end
