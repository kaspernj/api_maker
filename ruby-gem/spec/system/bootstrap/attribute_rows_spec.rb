...
describe "bootstrap attribute rows" do
  ...
  it "renders the page" do
    ...
    wait_for_expect do
      expect(wait_for_and_find(".user-row[data-user-id='#{user.id}'] .updated-at-column td", visible: false).text).to eq ""
    end

    # Should show 'updated at' so it shouldn't be empty
    wait_for_expect do
      expect(wait_for_and_find(".user-row[data-user-id='#{user_kasper.id}'] .updated-at-column td", visible: false).text).not_to eq ""
    end
  end
end
