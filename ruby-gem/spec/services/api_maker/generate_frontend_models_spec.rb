  ...
  describe "#perform" do
    it "generates one model file per resource with typed methods" do
      Dir.mktmpdir do |tmp_dir|
        ...
        expect(task_content).to include("import AccountModel from \"./account.js\"")
        expect(task_content).to include("import ProjectModel from \"./project.js\"")
        expect(task_content).to include("class Task extends BaseModel")
        expect(task_content).to include("static modelClassData()")
        expect(task_content).to include("@returns {import(\"@kaspernj/api-maker/build/collection.js\").default<typeof Task>}")
        ...
        expect(task_content).to include("const modelClass = ProjectModel")
