  ...
  describe "#perform" do
    it "generates one model file per resource with typed methods" do
      Dir.mktmpdir do |tmp_dir|
        ...
        expect(task_content).to include("export default class Task extends BaseModel")
        expect(task_content).to include("static modelClassData()")
        expect(task_content).to include("@returns {typeof modelClassData}")
        ...
        expect(task_content).not_to include("return super.ransack(query)")
        expect(task_content).not_to include("static select(select)")
        expect(task_content).not_to include("return super.select(select)")
        expect(task_content).not_to include("static find(id)")
        expect(task_content).not_to include("return super.find(id)")
        expect(task_content).not_to include("static all()")
        expect(task_content).not_to include("\nexport default Task\n")
        ...
        expect(task_content).to include("ensureProjectLoaded()")
        expect(task_content).to include("return this.ensureAssociationLoaded(\"project\")")
        ...
        expect(task_content).to include("ensureCommentsLoaded()")
        expect(task_content).to include("/** @returns {Promise<import(\"./user.js\").default | null>} */")
        expect(task_content).to include("ensureUserLoaded()")
