  ...
  describe "#perform" do
    it "generates one model file per resource with typed methods" do
      Dir.mktmpdir do |tmp_dir|
        ...
        expect(task_content).to include("typedef {typeof Task & {")
        expect(task_content).to include("}} TaskStaticType")
        expect(task_content).to include("/** @type {TaskStaticType} */")
        expect(task_content).to include("const TaskTyped = Task")
        expect(task_content).to include("export default TaskTyped")
