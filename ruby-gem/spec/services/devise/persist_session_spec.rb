...
describe Services::Devise::PersistSession do
  ...
  let(:request) { instance_double(ActionDispatch::Request, env: {"warden" => warden}, session: {}) }
  ...
  it "falls back to the stored warden session key when current_user is not hydrated yet" do
    ...
    request_with_session = instance_double(ActionDispatch::Request, env: {"warden" => warden}, session: request_session)
