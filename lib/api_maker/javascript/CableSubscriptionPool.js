...
export default class ApiMakerCableSubscriptionPool {
  ...
  onReceived(data) {
    var modelName = inflection.camelize(inflection.singularize(data.model.type))
    var modelId = data.model.id
    var modelClass = require(`./Models/${modelName}`).default
    var modelInstance = new modelClass(data.model.attributes)
    var subscriptions = this.props.subscriptions

    if (data.type == "update") {
      for(var subscriptionKey in this.props.subscriptions[modelName]["updates"][modelId]) {
        var subscription = this.props.subscriptions[modelName]["updates"][modelId][subscriptionKey]
        subscription.onReceived({model: modelInstance})
      }
    } else if (data.type == "event") {
      for(var subscriptionKey in this.props.subscriptions[modelName]["events"][data.event_name][modelId]) {
        var subscription = this.props.subscriptions[modelName]["events"][data.event_name][modelId][subscriptionKey]
