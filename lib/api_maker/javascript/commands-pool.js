    ...
    Api.requestLocal({path: url, method: "POST", rawData: formData}).then(response => {
      for(var commandId in response.responses) {
        ...
        if (commandResponse.type == "success") {
          ...
          commandData.reject(new CustomError("Command error", {response: commandResponse.data}))
        } else {
          commandData.reject(new CustomError("Command failed", {response: commandResponse.data}))
        }
      }
    })
  }
