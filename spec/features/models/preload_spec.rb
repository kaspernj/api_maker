...
describe "model preload" do
  ...
  it "finds the model", :js do
    ...
    puts task_with_preload
    puts task_without_preload

    expect(task_with_preload.fetch("modelData").fetch("project").fetch("name")).to eq "test-project"
    expect(task_without_preload.fetch("modelData").fetch("project")).to eq nil
  end
end
